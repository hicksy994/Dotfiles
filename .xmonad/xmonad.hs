...
myLayoutHook = smartBorders $ noBorders $ avoidStruts $ bsp ||| tall ||| tabs
  where bsp = named "BSP" $ reflectHoriz $ addTopBar $ gaps [(U,7), (R,7), (D,7), (L,7)] $ spacing 7 emptyBSP
        tall = named "Tall" $ addTopBar $ gaps [(U,7), (R,7), (D,7), (L,7)] $ spacing 7 $ Tall 1 (3/100) (1/2)
        tabs = named "Tabbed" $ reflectHoriz $ gaps [(U,14), (R,14), (D,14), (L,14)] $ tabbed shrinkText myTabTheme
...
myKeys =
    ...
    , ((myModifier .|. shiftMask, xK_h), namedScratchpadAction scratchpads "htop")
